...
def optimize_SARIMAX(parameters_list,train_set,val_set):
    ...
    for param in tqdm_notebook(parameters_list):
        try: 
            model=sm.tsa.statespace.SARIMAX(train_set["total_charge"],exog=train_set[["minor_holiday",'major_holiday','observed_holiday','is_extended',"count_weekend"]],order=(param[0],param[1],param[2]),seasonal_order=(param[3],param[4],param[5],param[6])).fit(disp=-1)
            fore1=model.predict(exog=val_set[["minor_holiday",'major_holiday','observed_holiday','is_extended',"count_weekend"]],start=train_set.shape[0],end=train_set.shape[0]+len(val_set)-1)
            fore=np.array(fore1)
...
#            val_mape=(.5*round((mean_absolute_percentage_error(val_set["total_charge"][0],fore[0])),2))+(.3*round((mean_absolute_percentage_error(val_set["total_charge"][1],fore[1])),2))+(.2*round((mean_absolute_percentage_error(val_set["total_charge"][2],fore[2])),2))
...
def sarimax_model(train_set,val_set,test_set,be_name,set_no):    
...
#    train_set,val_set,test_set=new_df_monthly[:-6],new_df_monthly[-6:-3],new_df_monthly[-3:]
    ...
    fore_val=best_model.predict(exog=val_set[["minor_holiday",'major_holiday','observed_holiday','is_extended',"count_weekend"]],start=train_set.shape[0],end=train_set.shape[0]+len(val_set)-1)
    ...
    for p1, d1, q1, P1, D1, Q1, s1 in result_table.parameters:
        try:
            if c > 5:
                break
            best_model_overall=sm.tsa.statespace.SARIMAX(overall_train["total_charge"],overall_train[["minor_holiday",'major_holiday','observed_holiday','is_extended',"count_weekend"]],order=(p1,d1,q1),seasonal_order=(P1,D1,Q1,s1)).fit(disp=-1)
            if best_model_overall.aic < res_aic_avg:
                c=c+1
                fore_test=best_model_overall.predict(exog=test_set[["minor_holiday",'major_holiday','observed_holiday','is_extended',"count_weekend"]],start=overall_train.shape[0],end=overall_train.shape[0]+len(test_set)-1)
    ...
    layout = go.Layout(go.Layout(title='Billing Entity : {} <br>Train MAPE: {} Val MAPE: {} Test MAPE: {} <br>SARIMAX(p,d,q)(P,D,Q,s):({},{},{})({},{},{},{})'.format(be_name,round(result_table.train_mape[0],2),round(result_table.val_mape[0],2),test_results,p, d, q, P, D, Q, s),yaxis=dict(title="Monthly Charges", zeroline=False),
                                 xaxis=go.layout.XAxis(
                                     title="Month-Year",
                                 ), boxmode='group'))
    fig = go.Figure(data=data,layout=layout)
    plotly.offline.plot(fig, filename=graph_path_tso+str(set_no)+"_sarimax", image='png')
