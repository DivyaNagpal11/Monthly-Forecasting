...
def optimize_VARMAX(parameters_list,train_set_transformed,train_set,val_set,scaler):   
    ...
    for param in tqdm_notebook(parameters_list):
        try:
            model=VARMAX(train_set_transformed[["total_charge","footfall"]], exog=train_set.reset_index()[['minor_holiday','major_holiday', 'observed_holiday', 'is_extended', 'count_weekend']], order=(param[0],param[1])).fit(disp=-1)
            fore1=model.forecast(steps=len(val_set),exog=val_set[['minor_holiday','major_holiday', 'observed_holiday', 'is_extended', 'count_weekend']])
...
def varmax_model(train_set,val_set,test_set,be_name,set_no):
#    train_set,val_set,test_set,be_name,set_no=train,val,test,be_name,c
    ...
    fore_val= best_model.forecast(steps=len(val_set),exog=val_set[['minor_holiday','major_holiday', 'observed_holiday', 'is_extended', 'count_weekend']])
    ...
    for p1, q1 in result_table.parameters:
        try:
            if c > 3:
                break
            best_model_overall=VARMAX(overall_train_transformed[["total_charge","footfall"]], exog=overall_train.reset_index()[['minor_holiday','major_holiday', 'observed_holiday', 'is_extended', 'count_weekend']], order=(p1,q1)).fit(disp=-1)
            if best_model_overall.aic < res_aic_avg:
                c=c+1
                fore_test=best_model_overall.forecast(steps=3,exog=test_set[['minor_holiday','major_holiday', 'observed_holiday', 'is_extended', 'count_weekend']])
    ...
    layout = go.Layout(go.Layout(title='Billing Entity : {} <br>Train MAPE: {} Val MAPE: {} Test MAPE: {} <br>VARMAX(p,q):({},{})'.format(be_name,round(result_table.train_mape[0],2),round(result_table.val_mape[0],2),test_results,p,q),yaxis=dict(title="Monthly Charges", zeroline=False),
                                 xaxis=go.layout.XAxis(
                                     title="Month-Year",
                                 ), boxmode='group'))
    fig = go.Figure(data=data,layout=layout)
    plotly.offline.plot(fig, filename=graph_path_tso+str(set_no)+"_varmax", image='png')
