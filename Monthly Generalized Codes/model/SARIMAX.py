...
class SARIMAX:
    ...
    def optimize_SARIMAX(self,parameters_list,train_set,val_set,exog_columns):
        ...
        for param in parameters_list:
            try: 
                ...
                val_mape=round(self.mean_absolute_percentage_error(val_set[self.target_value],fore1),2)
    ...
    def run_model(self):    
        print("in sarimax")
        print(self.target_item,self.target_items)
...
#        self.test=self.test.set_index(self.target_dates)
        ...
        result_table, best_model = self.optimize_SARIMAX(self.parameters_list,self.train[:-(int(self.n_periods))],self.train[-(int(self.n_periods)):],exog_columns)
...
#        print("in sarimax fitted mean",fitted_mean)
        if self.walk_forward == "True":
            ...
            self.results.append([self.target_item,"SARIMAX",self.set_no,round(result_table.train_mape[0],2),round(result_table.val_mape[0],2),test_mape,test_set1,fitted_mean,one_month,two_month,third_month])
