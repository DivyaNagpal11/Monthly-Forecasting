# -*- coding: utf-8 -*-
"""
Created on Sun Jul 19 01:35:06 2020
...
class VARMA:
    ...
    def fit(self,data,param):
        return VARMAX(data[[self.target_value,self.other_target_value]], order=(param[0],param[1])).fit(disp=-1)
    ...
    def optimize_VARMA(self,parameters_list,train_set_transformed,val_set,scaler):   
        results=[]
        best_adj_mape = float('inf')
        for param in tqdm_notebook(parameters_list):
            try:
                model=self.fit(train_set_transformed,param)
    ...
    def run_model(self):
        print("In varma")
#        print("params",self.parameters_list)
        scaler = MinMaxScaler()
...
#        print("transformed",train_set_transformed.head(5))
        ...
        result_table, best_model = self.optimize_VARMA(self.parameters_list,train_set_transformed,val_set,scaler)
...
#        fitted_values=scaler.inverse_transform(best_model.fittedvalues)[:,0]
        ...
        for param in result_table.parameters:
            try:
                if c > 3:
                    break
                best_model_overall=self.fit(overall_train_transformed,param)
                if best_model_overall.aic < res_aic_avg:
                    c=c+1
                    fore_test=best_model_overall.forecast(steps=self.n_periods)
...
#        print("in varma fitted mean",fitted_mean)
        if self.walk_forward == "True":
            ...
            self.results.append([self.target_item,"VARMA",self.set_no,round(result_table.train_mape[0],2),round(result_table.val_mape[0],2),test_mape,test_set1,fitted_mean,one_month,two_month,third_month])
