...
def varma_model(train_set,val_set,test_set,be_name):
    ...
    for p1, q1 in result_table.parameters:
        try:
            ...
            if best_model_overall.aic < res_aic_avg:
                ...
                fore_test_ci=best_model_overall.get_forecast(steps=3)
...
#    trace5 = go.Scatter(x=test_set.index, y=test_set['total_charge'], mode='lines+markers',name="Actual values: Test", marker=dict(color="blue",size=9,line=dict(width=1)))
    ...
    layout = go.Layout(go.Layout(title='Billing Entity : {} <br>Model: VARMA <br>Train MAPE: {} Val MAPE: {} <br>VARMA(p,q):({},{})'.format(be_name,round(result_table.train_mape[0],2),round(result_table.val_mape[0],2),p,q),yaxis=dict(title="Monthly Charges", zeroline=False),
